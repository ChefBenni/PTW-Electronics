...
const int MaxCycleTime = 150;    // the number of the jumper7 pin                    - Low active  - pull-up
const int BrakeDelay = 100;    // the number of the jumper7 pin                    - Low active  - pull-up
const int BrakeTime = 100;    // the number of the jumper7 pin                    - Low active  - pull-up
const int DebounceTime = 200;    // the number of the jumper7 pin                    - Low active  - pull-up
...
//  LsState = digitalRead(LS_Input_Pin);          // reads triggerstate

  // checks if the trigger is pressed and magazine not empty
  if (MagState == HIGH && TriggerState == LOW) {
...
void semi()
{
  ...
  // cycle start
  LsState = HIGH;                                       // tell at start light barrier is not detected
  previousMillis = millis();                            // read start time before cycle start
  while (LsState == HIGH){                              // waits for full cycle
    LsState = digitalRead(LS_Input_Pin);                // reads in the gearposition from the light barrier
    unsigned long currentMillis = millis();             // reads current time and stores it to save timers
  ...
  // wait for trigger not pressed including debounce
  TriggerState = digitalRead(Trig_Sig_Pin);       // reads trigger state
...
void automode()
{
  ...
  while (TriggerState == LOW){      // cycle while trigger is pushed
    TriggerState = digitalRead(Trig_Sig_Pin);   // reads trigger state
    LsState = HIGH;                                       // tell at start light barrier is not detected
    previousMillis = millis();                            // read start time before cycle start
...
void problem()
{
  digitalWrite(FET_On_Pin, LOW);      // turns the turn MOSFET off
  delayMicroseconds(BrakeDelay);      // turn-off delay time to avoid shot-through
  digitalWrite(FET_Brake_Pin, HIGH);  // turns the brake MOSFET on for braking
  delay(BrakeTime);                   // wait for brake time
  digitalWrite(FET_Brake_Pin, LOW);   // turns the brake MOSFET off
